...
STATE_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "cron", "python_skills_state.json")
...
if __name__ == "__main__":
    ...
    if topic:
        total = len(TOPICS)
        print(f"🐍 Python 技能学习 · 第 {topic['day']}/{total} 天")
        print(f"**{topic['title']}**")
        print("")
        print(topic['content'] if mode == "morning" else topic['practice'])
